...
if __name__ == '__main__':
    ...
    model.compile(optimizer='adam',
              loss=loss_fn,
              metrics=['accuracy'])
    pic=np.random.randn(*(1000,224,224,3))
    
    # x_train = tf.data.Dataset.from_tensor_slices(pic)
    
    label=np.array([np.random.randint(1000)]*1000)
    # y_train = tf.data.Dataset.from_tensor_slices(label)
    
    
    model.fit(pic,label,epochs=5,batch_size=198)
