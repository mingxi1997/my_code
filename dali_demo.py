...
train_loader = get_imagenet_iter_dali(type='train', image_dir=root, batch_size=256, num_threads=4, crop=224, device_id=0, num_gpus=1)
print('start iterate')
record=[]
for i, data in enumerate(train_loader):
    start=time.time()
    images = data[0]["data"].cuda(non_blocking=True).reshape(256,3,224,224).half()
